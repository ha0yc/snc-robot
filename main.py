...
def run(driver, username, password, url, shoe_size, login_time=None, release_time=None,
        page_load_timeout=None, screenshot_path=None, html_path=None, select_payment=True, purchase=True,
        num_retries=None):
    ...
    while True:
        try:
            ...
            if select_payment:
                ...
                try:
                    click_jiesuan_button(driver=driver)
                except Exception as e:
...
def login(driver, username, password):
    ...
    wait_until_clickable(driver=driver, xpath="//button[@data-path='join or login']")

    LOGGER.info("Clicking login button")
    driver.find_element_by_xpath("//button[@data-path='join or login']").click()

    LOGGER.info("Waiting for login fields to become visible")
    wait_until_visible(driver=driver, xpath="//a[text()='使用电子邮件登录。']")
    driver.find_element_by_xpath("//a[text()='使用电子邮件登录。']").click()
    ...
    wait_until_visible(driver=driver, xpath="//a[@data-path='my account']")
...
def select_shoe_size(driver, shoe_size):
    ...
    wait_until_clickable(driver, xpath="//button[text()='43']", duration=10)

    LOGGER.info("Clicking size dropdown button")
    driver.find_element_by_xpath("//button[text()='43']").click()
    ...
    LOGGER.info("the size is selected")

def click_buy_button(driver):
    xpath = "//button[text()='加入购物车']"
...
def click_jiesuan_button(driver):
    xpath = "//button[@data-automation='checkout-button']"
...
def wait_until_clickable(driver, xpath=None, class_name=None, duration=1000, frequency=0.01):
    if xpath:
...
def wait_until_visible(driver, xpath=None, class_name=None, duration=1000, frequency=0.01):
    if xpath:
